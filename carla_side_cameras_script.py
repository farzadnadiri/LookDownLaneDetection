...
class World(object):
    ...
    def setup_no_shadows_weather(self):
        # Set weather parameters with supported attributes in CARLA 0.9.8
        weather = carla.WeatherParameters(
            cloudiness=0,  # Full cloud cover to minimize shadows
            precipitation=0.0,  # No rain
            sun_azimuth_angle=0.0,  # Position of the sun (0-360 degrees)
            sun_altitude_angle=90.0  # High altitude to reduce shadows
        )
    ...
    def restart(self):
        ...
        self.setup_side_mirror_cameras()
        actor_type = get_actor_display_name(self.player)
        self.hud.notification(actor_type)
        self.setup_no_shadows_weather()
    ...
    def setup_side_mirror_cameras(self):
        blueprint_library = self.world.get_blueprint_library()
        camera_bp = blueprint_library.find('sensor.camera.rgb')

        # Disable shadows
        # camera_bp.set_attribute('enable_postprocess_effects', 'false')
        # camera_bp.set_attribute('gamma', '3.5')

        # Transformations for the side mirror cameras
        left_camera_transform = carla.Transform(carla.Location(
            x=-0.3, y=-1.3, z=1.1), carla.Rotation(pitch=-90, yaw=-90))
        right_camera_transform = carla.Transform(carla.Location(
            x=-0.3, y=1.3, z=1.1), carla.Rotation(pitch=-90, yaw=90))

        front_camera_transform = carla.Transform(
            carla.Location(x=2.5, z=1.5), carla.Rotation(pitch=-10))

        # Spawn the cameras
        self.left_camera = self.world.spawn_actor(
            camera_bp, left_camera_transform, attach_to=self.player)
        self.right_camera = self.world.spawn_actor(
            camera_bp, right_camera_transform, attach_to=self.player)
        self.front_camera = self.world.spawn_actor(
            camera_bp, front_camera_transform, attach_to=self.player)

        # Set up video writers
        self.left_video_writer = cv2.VideoWriter(
            'left_side_mirror.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 9.0, (800, 600))
        self.right_video_writer = cv2.VideoWriter(
            'right_side_mirror.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 9.0, (800, 600))
        self.front_video_writer = cv2.VideoWriter(
            'front_camera.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 9.0, (800, 600))

        # Start listening to the camera feed
        self.left_camera.listen(lambda image: self.save_video(
            image, self.left_video_writer))
        self.right_camera.listen(lambda image: self.save_video(
            image, self.right_video_writer))
        self.front_camera.listen(lambda image: self.save_video(
            image, self.front_video_writer))

    last_frame_time = time.time()

    def save_video(self, image, video_writer):
        current_time = time.time()
        elapsed_time = current_time - self.last_frame_time

        if self.recording and elapsed_time >= 1.0 / 9.0:
            image.convert(cc.Raw)
            np_image = np.frombuffer(image.raw_data, dtype=np.uint8)
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        vehicles = world.world.get_actors().filter('vehicle.*.car')
        self._info_text = [
...
def main():
    argparser = argparse.ArgumentParser(
        description='CARLA side mirrors control')
