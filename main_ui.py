...
from hsv_controls import create_hsv_controls, save_hsv_values, load_hsv_values, reset_hsv_values
...
save_button = ttk.Button(window, text="Save", command=lambda: save_hsv_values(log_func, mode_selection))
save_button.place(x=600, y=380, width=100, height=30)

load_button = ttk.Button(window, text="Load", command=lambda: load_hsv_values(log_func, mode_selection))
...
# Start video updates for all frames using the same capture object
update_all_frames(cap, frame1, frame2, frame3, frame4, log_func, distance_label)
